...
exhibited by existing compilers, and limiting the scope of both. The cost of
doing so (in a consistent manner\sidenote{None of the memory models discussed
are consistent with all observed behaviour, and it is not clear that there is a
consistent one which would be.}) is that \kl{PNVI-ae-udi}\sidenote{Provenance
    not tracked via integers, for addresses which are exposed and provenances
user-disambiguated. I will explain this imminently.} (the provenance-aware
memory object model for C17, as detailed in technical standard ISO/IEC TS
6010:2025), is a complex minefield of \kl{UB} no human should be expected to
abide by unassisted.
...
are \emph{not} completely abstract.  Compilers must also be conservative in
such assumptions. \emph{Despite a lack of dataflow between} \cinline{k} and
\cinline{j}, uncommenting line 6 would make the program defined and print 7. I
will explain the precise rules justifying this surprising fact
...
This means that the disambiguation would need to be tracked in the
...
\begin{itemize}
    \item \textbf{Unnecessarily expressive.} It is difficult to think of any
        ...
        that idiom matters for modern code''~\sidecite{gustedt2021provenance}.
...
out in C++17 with the introduction \mintinline{cpp}{std::byte}, which is
