...
\begin{mathpar}
  \inferrule[{[VIP: $\textbf{intFromPtr}\langle\tau\rangle((@i, x)) = \mathit{Loc}\,(@i, x)$]}]{%
    ...
    (A, M) \rightarrow (A, M)}
\end{mathpar}\caption{The \kl{VIP} rules to cast an integer into a pointer which
...
\begin{mathpar}
  \inferrule[{[VIP: $\textbf{arith\_bin\_op}(\oplus, x_1, x_2) = x$]}]{%
    ...
    (A, M) \rightarrow (A, M)}
\end{mathpar}\caption{\kl{VIP} arithmetic operations on
...
\begin{mathpar}
  \inferrule[{[VIP: $\textbf{copy\_alloc\_id}(x, (@i, \_)) = (@i, \mathrm{to\_int}(x))$]}]{%
    ...
    (A, M) \rightarrow (A, M)}
\end{mathpar}\caption{\kl{VIP} \cinline{copy_alloc_id} primitive to create a
...
\begin{mathpar}
  \inferrule[{[PNVI-ae-udi: $\textbf{eq\_ptrval}(p_1, p_2) = b$]}]{%
    % \pi_1 =_S \pi_2 \eqdef \pi_1 = \iota_1 \wedge \pi_2 = \iota_2 \wedge S(\iota_1) = S(\iota_2) = \{ i \} \vee \pi_1 = @i_1 \wedge \pi_2 = @i_2 \wedge i_1 = i_2 \\\\
    % \mathrm{ambiguous}(p_1, p_2) \defeq
    %     p_1 = (\pi_1, a_1)
    %     \wedge p_2 = (\pi_2, a_2)
    %     \wedge a_1 = a_2
    %     \wedge \\\\
    {\begin{cases}
        b = \mathsf{true} & \mathrm{if}\ p_1 = p_2 = \mathsf{NULL} \\
        b = (a_1 =_{\mathbb{Z}} a_2) & \mathrm{if}\ {
            \left(
                \begin{array}{l}
                {\color{violet} p_1 = (\iota_1, a_1) \wedge p_2 = (\iota_2, a_2) \wedge S( \iota_1 ) = S ( \iota_2 ) = \{ i \} } \\
                \vee p_1 = (@i_1, a_1) \wedge p_2 = (@i_2, a_2) \wedge i_1 = i_2
                \end{array}
                \right) } \\
        b = \mathsf{false} & \mathrm{if} \\
    \end{cases}}
  }{%
    (A, M) \rightarrow (A, M)}
\end{mathpar}
