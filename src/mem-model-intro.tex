...
point to a live allocation and (b) can alias any other live allocation in scope.
...
\cinline{f}, which takes an integer \cinline{i} as an argument, assigns the
number 5 to a local \cinline{j}, and then prints it. Interspersed are some
seemingly irrelevant casts of the pointer \cinline{&j}, to a new local integer
\cinline{k}, and a cast of the argument \cinline{i} to a new local pointer
\cinline{p}, finishing with a write to \cinline{*p}. The function \cinline{f}
is called from \cinline{main} with a value \cinline{ADDRESS_PFI_1I}, this is a
``guess'' of the location of where \cinline{f}'s local variable \cinline{j}
will be allocated during execution.
...
\begin{figure*}
    ...
    .. \; a + n - 1]$, and the second rule should have $p = (\mathsf{@empty},
    x)$ (TODO\@: fix these).}\label{fig:pnvi-ival-to-pval}
\end{figure*}
