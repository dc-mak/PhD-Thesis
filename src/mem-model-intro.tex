...
\begin{figure}
\begin{mathpar}
  \inferrule[{[Label: $\textbf{ptrFromInt}\langle\tau\rangle(x) = (@i, x)$]}]{%
    \exists \, i.\ A(i) = (n, a, \mathsf{alive}, \mathsf{\color{blue}{exposed}}) \wedge x \in [a \ldots a + n - 1]%
  }{%
    (A, S, M) \rightarrow (A, S, M)}
  \\
  \inferrule[{[Label: $\textbf{ptrFromInt}\langle\tau\rangle(x) = (@\mathsf{empty}, x)$]}]{%
...
\begin{figure}
\begin{mathpar}
  \inferrule[{[Label: $\textbf{ptrFromInt}\langle\tau\rangle(x) = ({\color{violet}\iota}, x)$]}]{%
