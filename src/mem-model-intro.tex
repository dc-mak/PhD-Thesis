...
\begin{figure}
    ...
    \small%
    \raggedright%
    \cngrammarcompressed{.8\textwidth}{%
...
bytes.\sidenote{There is an additional component, an optional \intro{pointer
    ...
    been overwritten. Condition (a) relies on the rule for stores forbidding
    unaligned writes for pointers. Condition (b) relies integers being stored
    with $@\mathsf{empty}$ provenance at the memory byte level. The latter
    includes \cinline{memcpy} for parts of pointers, which the standard
    specifies that as a copy of characters (which are integer values). This
    means a (non-$@\mathsf{empty}$) provenance mismatch in the bytes of
    pointers is impossible.}\label{sn:ignore-ptr-index}
