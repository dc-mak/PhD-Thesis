...
\cinline{f}, and passes that guess in as an argument \cinline{f(j)}. Inside the
...
\begin{tikzcd}[cramped,sep=small]
	& {(@i, @j)} &&& {\{\,\mathsf{true}\,\}} \\
	{(@i, @\mathsf{empty})} && {(@\mathsf{empty}, @j)} & {\{\,\mathsf{true},\mathsf{false}\,\}} && {\{\,\mathsf{true},\mathsf{false}\,\}} \\
	& {(@\mathsf{empty}, @\mathsf{empty})} &&& {{\color{red}\{\,\mathsf{false}\,\}}}
	...
	\arrow[color={red}, from=3-5, to=2-4]
	\arrow[color={red}, from=3-5, to=2-6]
\end{tikzcd}%
\caption{Hasse diagram showing why the {\color{red}$\pi_1 \neq \pi_2$}
    condition is not monotonic with between the source domain (pairs of
    provenances) on the left and target domain (sets of possible boolean
    values) on the right. In this case, fewer elements represent more precise
    information, so the set $\{\,\mathsf{true},\mathsf{false}\,\}$ should be the
    \emph{bottom} element rather than the top.}\label{fig:ptr-eq-hasse}
\end{figure*}
...
per~\cref{fig:ptr-eq-hasse}, but need opposite if-conditions (\cinline{p != q}
