...
tracking where a pointer came from. With this (because C allows casting
pointers to and from integers) the question naturally arises about whether this
provenance should be tracked via integers or not. Unfortunately, that is not
the only choice point involved in coming up with an acceptable memory object
model; \sidetextcite{memarian2022cerberus} documents and explains the
trade-offs between this and other choices.

Any set of choices must nagivate the tension between allowing low-level idioms
...
which are exposed and provenances user-disambiguated. I will explain this
imminently.} the provenance-aware memory object model for
C17,\sidenote{Detailed in technical standard ISO/IEC TS 6010:2025,
and~\textcite{gustedt2022provenance}.}, manages to navigate this admirably.
Despite the fact that none of the memory models discussed are consistent with
all observed behaviour (and it is not clear that there is a consistent one
which would be), \kl{PNVI-ae-udi} allows for many extant low-level idioms and
compiler optimisations. However, it does so by sacrificing
programmer-friendliness: following its rules unassisted would stretch the
cognitive capacity of any human.
...
My contribution in this part is the design, fomalisation and implementation
of \kl{CN-VIP}, the memory model of \kl{CN}.

In \fancyref{chap:mem-model-explained}, I will provide some background to
\kl{VIP}, and then explain the choices it makes regarding its representation of
pointers and integers, and the idioms it supports. I will also explain
important and subtle complexities around round-trip casts, \cinline{memcpy} and
\cinline{memcmp}, which are handled in \kl{VIP}/\kl{PNVI-*} test-suite, but not
discussed. Throughout this, I will explore other options for pointer and
integer representations, and how they trade expressiveness and simplicity.
Lastly, I will summarise the choices I made for \kl{CN-VIP} amongst those
options.

In \fancyref{chap:cn-vip}, I will explain how I integrated \kl{CN-VIP} into
...
novel structure for heaps, which abstracted away some details, as mentioned in
(\fancyref{sec:res-terms}).

In the same chapter, I will also explain how I implemented \kl{CN-VIP} in
\kl{CN}, by incrementally developing, gating and deploying the complex changes
to the memory model. By the time I had designed and formalised \kl{CN-VIP},
\kl{CN} had users, a tutorial, a test-suite and a continuous integration
pipeline, so I had to be careful to ensure a smooth transition.

In \fancyref{chap:epilogue-cn-vip}, I will explain how switching over to
\kl{CN-VIP} (without round-trip casts) impacted the annotations and performance
on existing \kl{CN} tests and tutorial examples. I will then conclude by
discussing how \kl{CN} handles proofs of lemmas, which depend on the specifics
of the memory object model, and discuss how this work unexpectedly sheds light
on how we can improve the foundational correctness of \kl{CN} itself.
...
pointers, integers and memory itself are left abstract, and memory actions
(\cref{fig:mem-action-intf}) and operations (\cref{fig:mem-op-intf}) are
...
\begin{figure}
    ...
    $\cnkillXXkind{}$
    \endgroup%
    \caption{\kl{ResCore} memory action interface, a simplified subset of the
        \kl{Core} memory action interface.}\label{fig:mem-action-intf}
\end{figure}
...
\begin{figure}
    ...
    \endgroup%
    \caption{\kl{ResCore} memory operation interface, a simplified subset of
        the \kl{Core} memory operation interface.}\label{fig:mem-op-intf}
\end{figure}

We start by discussing \kl{provenance}. To optimise our code, compilers would
...
Does this program have \kl[UB]{undefined behaviour}, and if so, where and why? If it is
compiled and executed, what will it print?
...
resulting in \cinline{p} having $@\mathsf{empty}$ provenance. It can print 5,
...
$i$. Allocation IDs are allocated fresh globally for every new allocation, and
thus guaranteed to be unique. An \intro{allocation history} $\mathit{A}$ is a
partial map indexed by allocation IDs, which tracks their base address, size,
...
\subsection{Ae: Considering only address-exposed allocations for
...
Address-exposure is why uncommenting line 6
...
\emph{un}exposed, so the pointer \cinline{int *p} cast from the integer
...
i} \emph{is} exposed, so the pointer \cinline{int *p} cast from the integer
...
via an additional finite map $\mathit{\color{violet}{S}}$. Thus in the case
where an integer is cast to a one-past pointer on the boundary between two live
allocations, its provenance is assigned to be symbolic, containing both
allocations' IDs (\cref{fig:pnvi-ival-to-pval-symbolic}).
...
The basic idea behind \kl{VIP} is to require all integer-to-pointer casts to be
annotated with the pointer whose provenenace it must recover. This relaxed to
support a limited form of the round-trip property: only integer values which
were cast from pointers, with no intervening operations, can be round-trip cast
back to pointers. Any intervening operation on that integer (including adding
0) will result in an $\mathsf{@empty}$ provenance when cast to a pointer.
...
\kl{PNVI-ae-udi}. This is because fundamentally \kl{VIP} tracks exposure of
specific \emph{pointer values} rather than of whole allocations. Disallowing
intervening operations (even if the integer value remains within the bounds of
the same allocation) removes the need to do a search and bounds check of all
live allocation when casting back (I will discuss how \kl{VIP} recovers
allocation IDs in such cases soon). This also means that a boundary pointer is
...
PNVI-ae-udi for it to be a sound abstraction; this is achieved by
...
\emph{relevant}\sidenote{See~\fancyref{sec:linearity-manage-resources} for the
definitions of \kl{relevant}, \kl{linear}, and \kl{intuitionistic}.} fact:
forgetting this would enable disambiguating the pointer in inconsistent ways.
If the symbolic provenance of a boundary pointer is disambiguated with a memory
operation (a load, store, array offset, pointer differences, or kills), then
subsequent operations must not assume the provenance is unambiguated or
resolved the other way. Hence, disambiguation would need to be tracked in the
...
Exposure tracking has the following advantages over \kl{VIP}.
\begin{itemize}
    ...
    \item \textbf{Simpler (and backwards compatible) representation of
        ...
        as mentioned above, \kl{VIP} has a more complicated representation
    ...
    \item \textbf{Exposure is an intuitionistic fact.} Forgetting an exposure
        ...
        use with \cinline{copy_alloc_id}.
    \item \textbf{Consistent interface for casting integers to pointers.}
        \kl{VIP} permits either `plain' (round-trip) casts from integers to
        valid pointers, or casts with \cinline{copy_alloc_id}, whereas exposure
        tracking would consistently require a \kl{CN} annotation at each
...
respect to \kl{PNVI-ae-udi}, I ultimately decided that to outline its
design and defer implementation until it was necessary.
...
optimisation.\sidenote{\cinline{memcpy} does not expose allocations, but a
    ...
    question~\cite{gustedt2022provenance}. Additionally, the value of padding
...
complexity and performance penalties opt-in, it also (independently) matches
the convention set out in C++17 with the introduction
...
IR~\sidetextcite{lobo2026towards}, to represent uninitalised memory, and copy
and compare raw data. Lastly, as mentioned before, I deferred exposure tracking
...
This is both mathematically elegant, and beneficial for the the \kl{CN}
implementation because, in most cases\sidenote{Except for memory bytes,
see~\fancyref{subsec:smt-rep}.} $@\mathsf{empty}$ provenances can be
represented in the SMT solver problem as fresh unconstrained allocation IDs,
...
live allocation for the operation to not have \kl{UB}. For other operations, it
must still satisfy a bounds check, but whether an allocation is live is
irrelevant. There is no operation which requires knowing that an allocation is
dead.

In summary: bounds checks, some pointer operations and \coreinline{kill} all
require allocation information. Additionally, pointer operations and
\coreinline{kill} require the allocation be live (bounds checks do not). Only
\coreinline{kill} mutates existing entries allocation history (in \kl{VIP}).
...
predicates which take pointer arguments, almost always time a pointer will be
...
padding resource \cninline[breaklines]{take p = Padding<union u, z>(p)}, % chktex 36
