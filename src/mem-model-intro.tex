...
\subsection{Provenance in integers and bytes}
...
\begin{itemize}
    ...
    \item \textbf{Restrict the ability to perform round-trip casts to new C
        ...
        makes the user-visible memory object model more complex.\sidenote{For
            ...
            subtyping relation, so \cninline{Owned<uintptr_t> <:
            Owned<rt_uintptr>}.}\label{sn:optin-typedef-subtype}
    ...
    \item \textbf{Track allocation address exposure.} So that the provenance
        of a cast pointer can be recovered from ambient exposed allocations
...
non-deterministic true or false (\cref{fig:nd-ptr-eq}).
...
pointer-equality rule is not clear. The issue can be seen with checking
...
\emph{allocation token} \cninline{Alloc}. Though it does take a pointer as an
argument, unlike \cninline{Block}, \cninline{Owned} and user-defined
...
disjointness facts based on \coreinline{Owned} and \coreinline{Block}, multiple
...
Of course, I could force the symbolic values to be concrete, but the advantage
of using symbolic values (for example, for addresses) is that this gives us
...
If not addresses, should provenances be concrete? The main thing that the
memory object model does with provenances is check whether they are equal,
unequal (for non-deterministic pointer equality) and use the
non-$@\mathsf{empty}$ ones (allocation IDs) as keys to look up in the
allocation history.

The $@\mathsf{empty}$ provenance must be represented with a special
constructor, because casting from any integer to a pointer in \kl{VIP} must
result in \emph{the same} empty provenance. Using an \emph{unconstrained}
variable would not work because in the pointer equality rule you need to be
able to test whether two provenances \emph{are distinct} (for the
non-deterministic case), but in every other case you need to be able to test
whether two provenances \emph{are equal}. If I did not support
non-deterministic pointer equality, then I could use a (single, global)
unconstrained variable.

And also unspecified values in padding bytes!
Representation of null pointer \cninline{ptr_eq(x, NULL) || !addr_eq(x, NULL)}. % chktex 26 chktex 36

Total functions, partial operations (array/member shift).
