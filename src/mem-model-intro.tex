...
unlike \cninline{W}, \cninline{RW} and user-defined predicates
which are restricted to pointers only.\sidenote{I need to update the
typing rules to reflect this.}

Unlike pointer disjointness facts based on \coreinline{RW} and
\coreinline{W}, I do not derive allocation or ID disjointness facts from
...
Ownership \cninline{take u = RW<union u>(p)} would mean \cninline{u: array % chktex 36
byte}. If one wishes to write or read a field, then the ownership would need to
