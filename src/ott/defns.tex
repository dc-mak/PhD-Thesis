...
In this document, I formalise and prove type safe Kernel CN:\ a formalism for
CN which assumes that universal quantifiers are explicitly instantiated, that
all existential quantifiers have explicit witnesses, and all resource
manipulations have proof terms with linear types. It does not require proof
terms for the logical properties, since by construction all of the entailments
fall into (decidable) SMT fragment. It is a bidirectional type system for
ResCore, a let-normal version of Cerberus' Core calculus.

The lack of inference makes it a simpler language for which to prove type
soundness, whilst still demonstrating all the key ingredients mentioned above.
Since it handles the majority of C, the entire system is very large, and so I
will only provide commentary on the highlights.

I also formalised an elaboration procedure to explain CN's resource
inference scheme, though I have not embarked on any proofs about it.
