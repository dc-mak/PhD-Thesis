...
exponential blow-up. Inputs are constraints $\Phi$, resources $\mathcal{R} , r
{:} \ldots$ and the requested resources $\cnkw{if}\ \cnnt{pce}_2\ \ldots$;
Outputs are the remaining lookup $\mathcal{R}'$, and the leftover context
$\mathcal{R}''$ after a partially successful lookup.

{\small%
\[
    \Phi \vdash (\cnkw{if}\ \cnnt{pce}_1\ \cnkw{then}\ \mathcal{R}_{21}\ \cnkw{else}\ \mathcal{R}_{22})
                \mathrel{{\in}{?}}
                (\mathcal{R} , r {:} \cnkw{if}\ \cnnt{pce}_2\ \cnkw{then}\ \mathcal{R}_{11}\ \cnkw{else}\ \mathcal{R}_{12})
                \rightsquigarrow
                \outpol{\mathcal{R}'; \mathcal{R}''}
\]}
...
\cnkw{let}\ = t_{\cnkw{num}}; t'_{\cnkw{num}} \mid \\
...
\inferrule[]
    ...
    { ( \mathrm{Decls} ; \Gamma \vdash \cnkw{let}\ x = t_1 ; t_2 \Rightarrow \iota_2 ) \rightsquigarrow
    \\ \quad ( \mathrm{Decls}' ; \Gamma' \vdash \cnkw{let}\ x = t'_1 ; t'_2 \Rightarrow ( \beta_2 , b_1 \wedge (\cnkw{let}\ x = t'_1\ \cnkw{in}\ b_2) ) ) }
\]}
