...
In this chapter, I propose solutions to some of \kl{CN}'s more pressing
technical limitations.

\section{Inferring frames}\label{sec:inferring-frames}

Recall the \kl{Core} operators for expressing most control-flow: {\small
$\cnkw{run}$} and {\small $\cnkw{save}$}, first presented
...
\begin{enumerate}
    \item \textbf{Inlining slows down performance and duplicates
        ...
        The transformation from \kl{Core} with {\small $\cnkw{run}$}
        and {\small $\cnkw{save}$} to \kl{Core} with non-recursive runs
...
The reason for this is the \kl{Core} dynamics for the {\small $\cnkw{run}$}
operator: the current continuation $C$ is discarded, and the continuation
$C_\cnnt{id}$, associated with the label $\cnnt{id}$, is obtained from a
\emph{pre-computed} $\mathrm{labelmap}$, and resumed.
...
{\small$\cnkw{false} \wedge \cnkw{I}$}, even if, for example, a program is entering and
...
since \kl{CN} inlines all labels whose bodies do not recursively {\small
$\cnkw{run}$} to themselves.
...
{\small $\cnkw{save}\ \cnnt{id}\ (x\, {:}{=}\, \cnnt{pce})\ \cnkw{in}\ E$},
into \emph{operationally equivalent ones} using {\small $\cnkw{where}$}
expressions:
{\small\[
...
Labels $\cnnt{id}_i$ are in scope in \emph{entry} expression $E$, and in each
$E_i$, allowing for mutual
recursion, but are encapsulated from the enclosing expression. This is unlike
\kl{Core}'s {\small $\cnkw{save}$} operator, whose label is in scope of
the enclosing \emph{procedure}. Variables defined in entry expression $E$ are
not in scope
...
A {\small$\cnkw{where}$} expression is a context {\small$\cnnt{C} \,
{:}{=} \, \ldots \mid C\ \cnkw{where}\ l$}, and {\small$l$} is a short-hand for
the family of labels {\small$\cncomp{\cnnt{id}_i\ (x_i {:} \beta_i).\
E_i}{i}$}. When {\small$\cnkw{run}$}ning to a label, we search the context for
its definition $l$ and jump to its body with parameter substituted {\small
$[\cnnt{value} / x] E$}, keeping the enclosing context $C_1$, but discarding
the intervening context {\small$C_2$}, in a style reminiscent of checked
exceptions.

Note that, \sidetextcite{ghalayini2024denotational} stratify their language
into statements and expressions; their {\small$\cnkw{where}$} construct is a
statement, but here, a {\small$\cnkw{pure}(\cnnt{value})$} expression exits a
{\small$\cnkw{where}$} expression.
...
A consequence of the transformation of {\small$\cnkw{save}$}-\kl{Core} programs
to {\small$\cnkw{where}$}-\kl{Core} programs is that each label is either a
loop back-edge, a join-point from an if-expression, or a C source label. To
state typing rules for {\small$\cnkw{where}$}-expressions, we need two ingredients.
\begin{enumerate}
  \item \emph{Ordered} resource contexts {\small $\mathcal{W}$}, to mirror the
      {\small$\cnkw{where}$}-contexts in the dynamics. These will consist of
      \kl{linear} \kl{resource contexts} {\small $\mathcal{R}$}, separated by label
      definitions $l$ as ordering markers.
  \item Preconditions {\small$\cnnt{fun}$} and {\small$\cnnt{fun}_i$}
      for the entry expression {\small $E$}, and each label {\small
      $\cnnt{id}_i$} respectively, which end in
      {\small$\cnkw{false} \wedge \cnkw{I}$}.
\end{enumerate}
...
I can also sketch a typing rule for {\small$\cnkw{run}$}s. First, any
...
$\cnnt{spine}$ would be checked against the function type $\cnnt{fun}$
(obtained via a lookup of the label $\cnnt{id}$ in the ordered resource
context). Note, only the \kl{resources} up to the
{\small$\cnkw{where}$}-expression need to be consumed, everything above it is
...
\kl(CN){preconditions} {\small$\cnnt{fun}$} depends on the label type. A loop
...
The {\small$\cnkw{let}\ \cnkw{strong}\ \cnnt{pat}\ \cnkw{=}\ \cnnt{E}_1\ \cnkw{in}\
\cnnt{E}_2$} construct sequences all memory actions performed by {\small$\cnnt{E}_1$}
before those performed by {\small$\cnnt{E}_2$}. However, {\small$\cnkw{let}\ \cnkw{weak}$} only
sequences positive memory actions similarly; in other words, for {\small$\cnkw{let}\
\cnkw{weak}\ \cnnt{pat} = \cnnt{E}_1\ \cnkw{in}\ \cnnt{E}_2$}, any
negative memory actions performed by {\small$\cnnt{E}_1$} are unsequenced with respect
to all memory actions performed by {\small$\cnnt{E}_2$}. A \emph{race} occurs if
...
A footprint {\small$\cnnt{fp}$} of an action is the range of bytes in memory it touches.
Like actions, footprints can be positive {\small${\color{red}\cnkw{pos}(\cnnt{fp})}$}
or negative {\small${\color{red}\cnkw{neg}(\cnnt{fp})}$}. A set of such footprints is
called an annotation {\small${\color{red}A}$}, and I use the notation
{\small${\color{red}A^{-}}$} to denote all the negative footprints in {\small${\color{red}A}$}
and {\small${\color{red}A^{+}}$} for all the positive ones.

Negative actions step to a new {\small$\cnkw{delay}(\cnnt{actions}, \cnnt{E})$}
construct, where {\small$\cnnt{actions}$} is a set. Note that the heap {\small$h$}
does not change (yet).
...
a context {\small$\cnnt{C} \, {:}{=} \, \ldots \mid \cnkw{delay}(\cnnt{actions}, C)$}.
All {\small$\cnkw{delay}$}ed actions are negative, and all negative actions evaluate to
{\small$\cnkw{Unit}$} (since they are side effects) and so the result of the action is
...
The next ingredient is a {\small$\cnkw{disj}({\color{red}A}, E)$} construct, which is
used to check if {\small${\color{red}A}$} races with {\small$E$}.
...
The construct is used for weak sequencing, and is also a context {\small$\cnnt{C} \,
...
ones on the outside and the negative ones in a {\small$\cnkw{disj}$} ensures the
negative actions which \emph{were} part of evaluating {\small$\cnkw{pure}(\cnnt{value})$} are
treated as if unsequenced with {\small$E$}, \ie{} required to not race.
...
Nested {\small$\cnkw{delay}$}s can be combined by taking the union of their action sets.
...
Crucially, {\small$\cnkw{delay}$}s can be hoisted out of {\small$\cnkw{let}\ \cnkw{weak}$},
{\small$\cnkw{unseq}$}, {\small$\cnkw{disj}$} and annotated {\small$\prescript{\color{red}A}{}{E}$}
...
Notably absent, {\small$\cnkw{let}\ \cnkw{strong}$} or {\small$\cnkw{bound}$} expressions
\emph{do not} permit hoisting {\small$\cnkw{delay}$}s out of them. This forces any
...
{\small$\cnkw{delay}(\cnnt{actions}, E)$} is reminiscent of the outward
propagation of an algebraic effect~\sidecite{pretnar2015an}, and the barrier
behaviour of a {\small$\cnkw{let}\ \cnkw{strong}$} or {\small$\cnkw{bound}$}
...
indexing the points-to predicate $\cnkw{Owned}$ by the C type ($\tau$) and the
initialisation status ($✓$, $\uninitCross$), we track whether it is
...
become:
\begin{itemize}
    \item Negative stores consume an immediate {\small$\cnkw{Owned}$}, but return a
        delayed one (with usage 1).
    \item {\small$\cnkw{let}\ \cnkw{weak}$} marks all delayed, usage 1
        {\small$\cnkw{Owned}$}s returned by {\small$E_1$} with usage 0 for
        checking {\small$E_2$}.
    \item {\small$\cnkw{let}\ \cnkw{strong}$} marks all delayed, usage 0
        {\small$\cnkw{Owned}$}s returned by {\small$E_1$} with usage 1 for
        checking {\small$E_2$}.
    \item A bound expression changes all delayed {\small$\cnkw{Owned}$}s
...
\kl(CN){predicate} of sorts: $\cnnt{\#loc}\;(\ldots)$. In this case, it would
...
\emph{structural} equality, if $\cnnt{pce}_1 \leftrightarrow \cnnt{pce}_2$,
...
integers is straightforward: insert {\small$\cnnt{term} \mod 2^k$} as needed, as long
as one accepts the resulting expression may not being decidable (represented
with uninterpreted functions): $x \times y$ in bit-vectors will be
{\small$\mathrm{mul\_uf}(x,y) \mod 2^{64}$} in integers. We can apply this
...
Let $\beta$ be the \kl{base types} in the bounded bit-vector world. Let $\iota$
be the \kl{base types} in the unbounded integer world. Let {\small$\cnkw{num}
\mathrel{{:}{=}} \cnkw{int} \mid \cnkw{bit}$} be a parameter for the following.
Let {\small$\cnnt{term}_{\cnkw{num}}$} be the grammar of SMT terms,
...
Under this convention, terms {\small$t_{\cnkw{int}} {:} \iota$} and
{\small$t_{\cnkw{bit}} {:} \beta$}. I use {\small$b$} for formulas of boolean type:
{\small$b_{\cnkw{bit}} {:} \cnkw{bool}$}.

Let {\small$\mathrm{Decls}$} be a context with declaration types of \kl(CN){pure SMT functions}. The judgement
{\small$\mathrm{Decls} ; \Gamma \vdash t_{\cnkw{int}} \Rightarrow \iota$}  means that
under declarations {\small$\mathrm{Decls}$} and context {\small$\Gamma$}, expression in the
source integer world {\small$t_{\cnkw{int}}$} has type {\small$\iota$}. {\small$\mathrm{Decls}$} is a
map from function names to function types, {\small$\Gamma$} is a map from parameters
...
functions, also have named recursive constraints, named {\small$f_b$}.
...
I will use {\small$\rightsquigarrow$} to define a mapping from the integer
world to the pair of bounds-constraint and expression in the bit-vector world.
For example, the mapping between types, from {\small$\iota$} to
{\small$\beta$}, could be as below.
...
branch condition $t'_1$ is constrained by $b_1$, and the evaluation of each
...
Pure base types are {\small$\beta$}, resourceful ones could be
{\small$\cnkw{res}\,\beta$}, and the SMT-related ones could be
{\small$\cnkw{smt}\,\beta$}. Note that this can be merely an internal
...
Terms of type {\small$\cnkw{res}\,\beta$} can be interpreted as maps into \kl{separation
logic} propositions, {\small$\beta \rightarrow \mathsf{Prop}^\mathsf{SL}$}, as mentioned
in~\cref{fig:monad-sl}. Terms of {\small$\cnkw{smt}\,\beta$} are instead interpreted as
a \emph{pairs} {\small$\beta \times \mathsf{Prop}^\mathsf{SMT}$}. Consider the
aforementioned term grammar extended with {\small$\cnkw{assert}(b)$}. A full treatment
...
{\small$\cnkw{assert}(b)$} to {\small$\cnkw{default}\,\beta$}, but I present a sketch below.
