...
My contributions in this chapter are to sketch out some solutions to some of
\kl{CN}'s more pressing technical limitations.
...
\coreinline{run()} operator: the current % chktex 36
...
The solution\sidenote{Implemented in \href{https://github.com/rems-project/cerberus/pull/1005}{Cerberus\#1005}.}
is to augment the grammar of \kl{Core} to borrow an idea from an
inductive representation of SSA, based on work
...
label, we search the context for its defintion $l$ and jump to its body with
...
approach to typing would be to take advantage of the fact that negative actions
are only ever used for stores. Thus, in addition to indexing the points-to
predicate $\cnkw{Owned}$ by the C type ($\tau$) and the initialisation status
($✓$, $\uninitCross$), we track whether it is \emph{delayed} or an
\emph{immediate}. Next, we annotate each resource in the context with a usage
annoation (we need 0 and 1 at the very least). With that, the typing rules
become:
\begin{itemize}
    \item Negative stores consume an immediate $\cnkw{Owned}$, but return a
        delayed one (with usage 1).
    \item $\cnkw{let}\ \cnkw{weak}$ marks all delayed, usage 1 $\cnkw{Owned}$s
        returned by $E_1$ with usage 0 for checking $E_2$.
    \item $\cnkw{let}\ \cnkw{strong}$ marks all delayed, usage 0 $\cnkw{Owned}$s
        returned by $E_1$ with usage 1 for checking $E_2$.
    \item A bound expression changes all delayed $\cnkw{Owned}$s returned by
        its argument expression into immediate ones.
\end{itemize}
...
$\mathrm{mul\_uf}(x,y) \mod 2^{64}$ in integers. We can apply this
...
To solve this, I am now going to define precise what I mean by `integer world',
`bit-vector world', and then define a mapping from one expression in the
...
source integer world $t_{\cnkw{int}}$ has type $\iota$. $\mathrm{Decls}$ is a
map from function names to function types, $\Gamma$ is a map from parameters
...
functions, also have named recursive constraints, named $f_b$.
...
pair of bound-constraint and expression in the bit-vector world. For example,
the mapping between types, from $\iota$ to $\beta$, would be along the lines of
the below.
...
(\fancyref{sec:restriction-branching}), the two definitions could be combined
...
cost of verification and being usable by `systems programmers who proficient
in functional programming'. I showed how these engineering constraints informed the choice of
...
existing optimising compilers. I showed that we can simplify some of the
...
direction of more trustworthiness in \fancyref{sec:better-foundations}; I will
    ...
    \item \textbf{Perform a `mem2reg' pass on \kl{Core}.} Representing stack
        ...
        \begin{itemize}
            \item Implemented in
                ...
                resource context lookup code. It remains useful for usability.
        \end{itemize}
