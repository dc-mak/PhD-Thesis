...
\coreinline{let strong}, \coreinline{bound()}, \coreinline{par()}. % chktex 36
...
their effects.\sidenote{$\cnkw{let}\ \cnkw{strong}\ \cnnt{pat}\ \cnkw{=}\
    ...
    in other words, $\cnkw{let}\ \cnkw{weak}\ \cnnt{pat}\ \cnkw{=}\
    \cnnt{E}_1\ \cnkw{in}\ \cnnt{E}_2$, any negative memory actions
    ($\cnkw{neg}(\cnnt{action})$) performed by $\cnnt{E}_1$ race with all
    memory actions performed by $\cnnt{E}_2$.}
In this example, weak sequencing has no observable effect, and so it is
safe to read both as \coreinline{let strong}.
...
\coreinline{load()} of \cinline{&xs} (which in Core is just % chktex 36
\coreinline{xs}), a case-split on the loaded value (UB if it is
\coreinline{Unspecified}), a \coreinline{member_shift()}, and then another % chktex 36
\coreinline{load()}. The whole expression is bound to \coreinline{a_454}. % chktex 36
...
of \cinline{&ys} (which in Core is just \coreinline{ys}). The whole expression
is bound to \coreinline{a_461}.
...
`forgetting' an assumption is known as \intro{weakening}, and given the intended
...
characterisable as \kl{precise}~\sidecite{reynolds2008intro}.
