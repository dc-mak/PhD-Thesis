In \cref{sec:c-lang}, I discussed how competing forces of inherited portability
requirements, proximity to hardware, and the desire for more aggressive
optimisations are somewhat reconciled by complex and subtle choices in the
\kl{ISO} standard of C. I also mentioned that its nature as a prose document,
with natural language ambiguities and omissions, as well as divergence from C
as used \kl{de facto}, mean that its semantics are unreasonable for a human to
adhere to, and challenging to build into tools directly, without making some
sort of simplifying assumptions.

Existing program logic frameworks for C build a logic directly above a
language which is simplified with respect to the \kl{ISO}; they attempt to
retain as many C features (control flow, variable scoping, aliasing, loose
evaluation order, pointer manipulation rules) as feasible.

Given that \kl{CN}'s headline goal (\cref{sec:cn-intro}) is to work with
pre-existing C programs, which rely on many if not all of those subtle
...
Instead, \kl{CN} builds directly upon the \kl{Cerberus} executable and
empirically validated semantics for C~\sidecite{memarian2022cerberus}. \kl{CN}
benefits from both the \emph{accuracy} and the \emph{usability} of this
semantics for both \kl{ISO} and \kl{de facto} C. This is because Cerberus is
elaborated into a relatively small calculus \emph{\kl{Core}}, which translates
all of C's complexity into a first-order functional language with a few special
(but easy to understand and specify) constructs.

Additionally, \kl{CN} is intended to be used more like a \emph{type system} in
an IDE than a program logic in a proof assistant. Ideally, instead of seeing
intermediate goals in a sophisticated separation logic, and needing to be well
versed with a range of inference rules and automation tactics, a user sees
their C program, scattered with predictable and lightweight annotations in
comments, in an editor which either indicates success, or a clear and helpful
error message.
...
Aiming to be a type system means we want to minimise obvious annotations and
justify why the necessary ones are so. Thus, we need to be careful about
tracking the flow of information in the type system, using a \kl{bidirectional}
approach. We also need some sort of automation so as to not burden the
programmer with proving things like $1 + 1 = 2$. Similar to
VeriFast~\sidecite{jacobs2011verifast} and
Frama-C~\sidecite{kirchner2015frama}, \kl{CN} enlists the support of SMT
solvers to mitigate this. When trying to verify code against expressive
specifications, this could lead to non-termination, so \kl{CN} also restricts
the expressiveness of the assertion language, and the queries it sends to the
SMT solver. And given the importance of managing resources in C, the typing
discipline needs to be substructural.
...
My contribution is this part is formalising and proving type safety for
\kl{Kernel CN}: a formalism for \kl{CN} which assumes that universal
...
types. It does not require proof terms for the logical properties, since by
construction all of the entailments fall into (decidable) SMT fragment.
\kl{Kernel CN} is a bidirectional type system for \intro{ResCore}, a let-normal
version of \kl{Cerberus}' \kl{Core} calculus.

I also formalised an elaboration procedure to explain the principles of
\kl{CN}'s resource inference scheme, though I have not embarked on any proofs
about it.

In~\fancyref{chap:formal-background}, I expand upon the aforementioned
constraints and design decisions, highlight and explain the relevant features
of \kl{Cerberus} and \kl{Core}, and cover the necessary type systems background
to understand \kl{CN}'s syntax and \kl{Kernel CN}.

In~\fancyref{chap:kernel-grammar}, I will explain the grammar of types in
\kl{Kernel CN}, how they relate to the \kl{CN}'s syntax. I will also explain
the grammar of \kl{ResCore} and how that relates to \kl{Cerberus}'s \kl{Core}
by example.

In~\fancyref{chap:kernel-statics}, I put the novel, multi-faceted and large
setup presented so far to use, and explain the \kl{Kernel CN} typing rules for
\kl{ResCore}. The lack of inference makes it a simpler language for which to
prove type soundness, whilst still demonstrating all the key ingredients
mentioned above. Since it handles the majority of C, the entire system is very
large, and so I will only discuss the main features. I also show rules for
inferring the quantifier instantiations assumed by \kl{Kernel CN}.
...
to many questions around the implementation, centred around lifiting
restrictions in the \kl{CN} implementation or adding new features.
...
The components of \intro{Kernel CN} all have precedent in prior work; my main
new contribution is adapting and combining those ideas. In this chapter, I set out
\kl{CN}'s design goals and origins, recapitulate the disparate concepts used in
CN, and along the way discuss how they satisfy the aforementioned design goals.
...
(\cref{sec:cn-intro}) helps narrow down the large design space of verification
tools.
...
that security is paramount, however, because each device runs its own kernel
(up to half the code is not Android's version of Linux), updates are very
...
used. There is already a mature hypervisor which is part of the Linux kernel,
the Kernel-based Virtual Machine (KVM)\@. It is set up so that a host kernel
can dynamically allocate virtual machines for guests to run on, and protect the
...
but the host could be a compromised version of Android, so an attacker could
still get access to sensitive information.
...
runs \emph{underneath} the kernel,\sidenote{pKVM utilises the Armv8 exception
    ...
    is used for Android applications, EL1 is used for the kernel, EL2 is
used for the hypervisor, and, EL3 is not used.} and ships \emph{as part} of the
kernel image. This approach allows pKVM to commit to only handling implementing
a select few functions such as virtual memory management and remain very small,
...
also make it one that is used very widely. It is in this context that we and
Google saw the opportunity to verify a small hypervisor, \emph{on an ongoing
basis}. A one-and-done verification which takes an army of PhD students and
postdocs a few years to verify and is years out of date by the time is
developed would not be worth the investment; a tool which C kernel programmers
can understand, use and maintain proofs as they make changes to very important
security critical code would be.
...
\begin{itemize}
    \item Because kernel programmers wrote the code, and are intending to use
        conventional compilers to build and run it, we cannot rely on
        (hopefully sound) approximation to the semantics of C
    ...
    \item Because it will be used by kernel programmers, we want a story that is
        is accessible and acceptable to them. They are very smart and
        capable people, who do not have the time or support to get up to
        speed with separation logics and interactive proof assistants.
        Nor are they amenable to changing their development or company's
        product delivery workflows substantially, which would be the case
        for using interactive proof assistants. I conjecture that a ``fancier
        type system'' which runs as part of the \intro[CI]{continuous
        integration (CI)} pipeline has a better chance of being adopted in this
        context.
...
\emph{how} it captures this executable semantics. Unusually, it does so by
...
\kl{Cerberus} parses, and statically checks \cref{fig:perplexing-ub}, despite
the strange scoping and strange control flow which jumps \emph{into} a loop
body. However, it declares the program illegal when executing it because of
subtle rules regarding block scopes, object lifetimes and initialisation (as
explained in the caption). Whilst contrived, any C function which happens to
have exception handling and retry behaviour, and be longer than a programmer's
screen height, could make the same error. Cerberus handles C features such as
mutable and potentially aliasing variables, and the myriad other
\kl[UB]{undefined} or \kl{unspecified} behaviours, for example loose evaluation
order and coercions respectively, by making them all \emph{explicit} in the
syntax.
...
pure fragment (\cref{fig:pure-core-grammar}) and an effectful on
(\cref{fig:effectful-core-grammar}); the former is embedded into the latter.
...
Next are the constructs related to evaluation order. A more precise treatment
of evaluation order and footprints of memory actions and expressions is given
later in \fancyref{sec:weak-seq}. As per \sidetextcite{memarian2022cerberus},
\emph{Each use of an action is either ``positive'' (the default case) or
...
calling elaborated C functions and Core procedures (effectful functions)
...
\textcite{memarian2022cerberus} and I present key rules from it below.
It is a small-step transition from a configuration to either undefined
behaviour or a new configuration. Formally, this is $\langle h, E, \kappa
...
continuations with default arguments. This is the \cinline{goto}-like operator
...
When evaluating a \coreinline{save()} expression, its parameters % chktex 36
...
\coreinline{run()} operator, then they are instantiated % chktex 36
...
Formally, reaching a \coreinline{save()} expression via its enclosing % chktex 36
expression evaluates its default arguments (pure expressions) into values and
then substitutes those values into its expression.\sidenote{Since this does not
...
\textbf{The important aspect for our purposes is that while control flows into
...
\kl{Core} procedure, \kl{Cerberus} adds a single \coreinline{save()} % chktex 36
expression to delare a return label (here, \coreinline{ret_430}) \emph{at the
...
takes two arguments, an integer (for constraining the aligment of the
resulting pointer), and a C type. \coreinline{Ivalignof(τ)} is an % chktex 36
integer value constructor which takes as an argument a C type τ; in C, the
alignnemnt of a type is an implementation-defined value and using an abstract
...
Elsewhere, lines 15\textemdash{}25 are bound to \coreinline{a_432}. That is
...
67\textemdash{}68 are bound to \coreinline{a_447} to be used in
...
(UB if it is \coreinline{Unspecified}), a \coreinline[breaklines]{member_shift()}, % chktex 36
...
of \cinline{&ys}. Elsewhere, lines 83\textemdash{}84 bound to
\coreinline{a_461}.
...
which is then stored in at \cinline{&new_tail} (in Core, just
\coreinline{new_tail}).
...
the stores do not: they are classed as \intro{side effect}s.\sidenote{%
...
In particular, \kl{compositional}ity is just as important, if not more, as the
...
such transporting would be a major undertaking in itself. It might be achievable
...
\emph{accomplish} (\cref{sec:error-msgs}).

Another challenge is that though elaboration simplifies greatly the checked
language, it also increases the distance between the checked and the typed
language. This becomes problematic when attempting to relate failures in SMT
queries back to what users wrote, particularly when those queries are part of
an inference procedure, rather than checking C source assertions
(\cref{sec:counter-ex}).

This concludes the background related to \kl{Cerberus}; now I will discuss the
