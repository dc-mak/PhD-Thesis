...
the definitions \kl{Core} standard library are omitted, as are choices about
...
\begin{figure}
    ...
    \end{mathpar}
    \endgroup%
    \caption{Simple \intro{bidirectional} type, adapted
    ...
    one for \intro[synthesis]{synthesising} a type and one for checking it.
    %
    ...
    for it, and then checking whether $T_1 <: T_2$ so that it checks against a
    type $T_2$. Here, the subtyping relation $\cnkw{<:}$ is merely structural
    type equality $=$, but in the presence of refinement types, it
    would be logical entailment.}\label{fig:simple-bidir}
\end{figure}
...
allocation, so we keep \emph{exchange}. Sequent calculus rules for contraction,
