...
sanity check on the output of elaboration. It has not been used for a proof of
soundness with respect to the dynamic semantics, since such a theorem would be
just another sanity check (on the dynamic semantics).
...
introducing a form of dependency. However, we do not want to make programming
language itself more expressive, merely its type system. For example, we do not
wish to the ability for computation to depend on types (as in a \kl{dependent
type} system). In fact, we would like to add richer types in a way such that we
could erase them after type checking, and recover the original program and
dynamic semantics.
...
statements, then we need some sort of automation to dispatch proof obligations:
recall the examples shown in \cref{fig:call-incr}, where \kl{CN} deduces
\cinline[breaklines]{y <= MAXi32()} from the constraint \cinline[breaklines]{y
<= 100i32}.

However, once we have rich constraints being tracked by the type system, we
have a first-order logic with arithmetic, which is undecidable. There are type
systems for real-world programming languages (such as Hack or F$^*$) which are
undecidable, and so could just never terminate on some code, but have
automation which works fine in practice, so why does this point require any
...
\kl{counter-example}, which is used to explain
...
\mathsf{x} \mid \mathsf{x} \geq 5 \,\}$.

The subsumption rule is not syntax-directed, so we are free to apply it now.
Doing so, we have have $\{\, \mathsf{x} \mid \mathsf{x} > 0 \,\}$, because
$\mathsf{x} \geq 5 \Rightarrow \mathsf{x} > 0$. At this point, we cannot
statically rule out the \cinline{else}-branch of the condition, so we start
checking the one with the exception, instead of skipping over it as dead code.

Whilst quite contrived, this illustrates the point that we would like some sort
...
everywhere else from the bottom up. As explained in sections 4.4 and 4.6
in~\sidetextcite{dunfield2021bidir}, this is precisely the sort of problem a
bidirectional type system can solve. Moreover, it gives a clear principle of
when to apply the subsumption rule (call the SMT solver): at the boundary between
