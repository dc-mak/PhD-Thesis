...
runs \emph{underneath} the kernel,\sidenote{pKVM utilises the Armv8 exception
    ...
    is used for Android applications, EL1 is used for the kernel, and EL2 is
used for the hypervisor (EL3 is not used).} and ships \emph{as part} of the
...
These were supported in the implementation at one point,\sidenote{See
note~\ref{sn:new-inf}.} but stopped working due to a change in the resource
...
the formalisation. The \coreinline{par()} construct is for spawning % chktex 36
threads. The \coreinline{nd()} construct models non-determinism as % chktex 36
...
manipulate the heap or the stack, it is phrased purely as is expression
...
so is safe to read as \coreinline{let strong}.
