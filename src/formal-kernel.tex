...
material difference to soundness, but it appeals to my aesthetics and
reduces redundant lookups in the resource context, and avoids the need of
inventing default values for empty arrays in the SMT problem.} Lastly, the
$\mathsf{chop}$ operation splits an iterated predicate along a given index
guard $\mathit{iguard}$. As mentioned in \nameref{sec:monadic-syntax}, to
