...
(\S\ref{chap:buddy}), engineering for accurate source location information
(\S\ref{sec:error-msgs}), and MiniCN (\S\ref{chap:kernel-alternative}).
...
\begin{itemize}
    \item Adapt and categorise the PNVI/\kl{VIP} test-suite to \kl{CN-VIP}\@.
...
\begin{figure}[h]
    \centering
    \includegraphics[width=\textwidth]{figures/cnvip-performance.png}
...
Although \kl{CN} does not \kl{A-normalise} \kl{Core} any more
...
new type called \emph{MuCore}, and it is this type that \kl{CN} type-checks.
...
\begin{itemize}
    ...
    \item \ocamlinline{function_spec}. \kl{CN} annotations for pre- and
        ...
        function body.
    \item \ocamlinline{loop_spec}. \kl{CN} loop invariants, placed after the loop
...
\begin{enumerate}
    ...
    \item \kl{CN} transforms \kl{Core} into MuCore, during which process
        the remaining magic comments are parsed and desugared into the same.
\end{enumerate}

After this, the MuCore file, specifically all the \kl{CN}-related
...
\begin{itemize}
    \item \ocamlinline{Mu}, short for \ocamlinline{Mucore}. Module for the
        MuCore syntax tree OCaml type. Contains the type for
...
record of base address and size, and has no clauses, \ie{} cannot be unfolded.
...
\section{Array and member shifting}\label{sec:impl-array-member-shift}
...
bytes. These features were not required for most of the \kl{VIP} test-suite, nor
...
\kl{VIP} test-suite. It requires the address ranges to not wrap-around (lines
29\textendash{}31), and ownership of the two arrays (merely to get a hold of
their \kl{ghost representation}, lines 32\textendash{}33). It ensures ownership of
...
(\S\ref{sec:auto-unfold-functions}), any uses use of \cinline{memcmp} must deal
...
handle on the \kl{ghost representation} of the arrays. The key lines are 46 and
...
because of the definition of \sm{\mathrm{combine\_prov}} given
...
empty provenance is represented for pointers (\S\ref{subsec:smt-rep}). The value
