...
I modified in the following ways. % RESUME here
\begin{itemize}
...
With these options, the tree-carver sets up a ClangTool instance, which is
...
implements a \kl{buddy allocator}~\cite{knowlton1965fast} to manage the pages it
...
The major code change is easy to describe. The \intro{buddy allocator} maintains a
...
so that the pointers were stored intrusively, in the free pages themselves.
...
The \kl{pKVM} \kl{buddy allocator} is a challenging piece of code to understand and
...
operations such as coalescing two buddy pages of the same order, adding a page
or removing one. In addition to the addresses computable from an index, the
...
\kl{buddy allocator} proof. This was slow and tedious. It was also occasionally
...
adjustments to the invariants needed to be. Surprisingly, I could leave this
until later \textemdash{} the changes were close to dependency leaf nodes, and
the original design and use of the resource and logical predicate definitions
...
changes. Additional \kl{CN} features might have made such an update easier. For
example, better integration of function-like macros would name conversions
between physical and virtual addresses (instead of using plain arithmetic). Or,
numeric type-classes would make conflating physical and virtual address a base
type error in specifications. Fixing and then rebasing these changes, alongside
cosmetic changes, took one month.
...
The main predicate of the \kl{buddy allocator} claims a contiguous iterated
ownership of page metadata, and \emph{sparse} iterated ownership of the pages
...
division (rerouting to avoid the bottom path in~\cref{fig:buddy-negative-div}),
...
working up updating the \kl{buddy allocator} proof to an early version of the
...
vectors. In short, the change made verifying the \kl{buddy allocator} at at least
...
verification is complex, and learning it without building up intuition for
separation logic and how to operate \kl{CN} made the process unnecessarily
difficult. This has been remedied with the introduction of the \kl{CN
tutorial}~\sidecite{pulte2024tutorial}.

\paragraph{Tree carving is indispensable (completed).} Manually carving and
grafting the code took a significant amount of time (3 weeks), once again
demonstrating the need and usefulness of the tree carver as an integral part of
any usable C verification tool.

\paragraph{Functions should be checked in dependency order.}
From the get go, the default order in which \kl{CN} checked its functions was
unhelpful. Ideally, one would like it to start with leaf functions and then
work its way up, but instead the user must figure out and maintain the
dependency structure in their head when planning the proof effort, and manually
select the functions to verify one at a time.
...
such macros into logical (ghost) functions, but this is difficult because
macros can and do refer to global variables. Instead of converting macros to
logical functions, it may be more fruitful to simply expand their use in
...
feature for lifting a C function into a logical one, with the idea being that
writing a C function around a macro call is an option sometimes. All this being
said, even if by hand, the benefit of defining logical equivalents of
...
(worked-around).} I spent a bit of time removing magic constants prevalent in
the original proof. Part of this was just human inconsistency, part of this was
...
iterated predicates must be integer literals (rather than \cninline{sizeof<>}
...
lemmas everywhere. The changes required to the code were pervasive, and because
we incorrectly assumed the \kl{buddy allocator} verification would be easy to
port, there was no desire to deliver the feature piecemeal and opt-in. This was
a costly error, not only for updating the \kl{buddy allocator} verification,
but also for maintaining \kl{CN}'s speed.
...
into bit-vectors (\cref{sec:integers-bitvectors}).
...
failing, or which sub-term in a large expression was not well-formed. This also
...
mentally. A trick I learned from Thomas Sewell was to constrain integer values
to be small in the precondition of functions failing to verify. For instance,
set the physical/virtual offset to 0 and limit the pool to a tiny range of
small memory addresses. This makes counter-examples more manageable and should
be automated.
