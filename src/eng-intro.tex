So far I have discussed the formalisation of \kl{CN}
(\fancyref{part:formalisation}), the formalisation and implementation of its
memory model (\fancyref{part:mem-model}), \kl{CN-VIP}. In both those sections,
...
Now I will take a step back and focus more on what it takes to build
a \emph{usable} verification tool. Though not as theoretically intriguing or
elegant as some of the material in prior parts, this is where just as
much, if not more, of the difficulty lies in achieving \kl{CN}'s goal of
enabling kernel programmers to maintain proofs on implementations
(\cref{sec:cn-goals}).
...
C code into verification tools such as \kl{CN}. Lastly, I will show how,
extending prior work significantly, I built a Clang-based tool which meets
almost all the requirements we sought, enough to no longer be the primary bottleneck
to verifying code in large projects. Lastly, I will explain the
limitations of the current approach (struct fields, cross-translation-unit
carving), and ways this may be remedied in the future.

\section{Need for tree-carving}

Tree-carving is inspired by the idea of tree-shaking in dynamic languages.
...
It lives in the folder \href{https://github.com/rems-project/linux/blob/pkvm-core-6.4/arch/arm64/kvm/hyp/pgtable.c}{arch/arm64/kvm/hyp/pgtable.c}, and consists of some simple
...
Cerberus does not support. However, these features are not at
...
the construct in the parser, regardless of whether or not is used later,}
...
One may think that preprocessing would sidestep this issue, because unused
...
littered with unsupported features. The structure of the macro and its
...
repository, for which preserving file structure may be
