...
\begin{itemize}
    ...
    \item Designed for portability, relative simplicity (but with plenty of
        subtlety around pointers), ease of compilation, proximity to hardware/control.
    ...
    \item ISO standard vs.\ de facto C\@.
...
\begin{itemize}
    \item John Reynolds and Peter O'Hearn
    \item Pen and paper proofs about sequential, then concurrent imperative programming.
...
\intro{annotations} in comments demarcated by \cinline{this} and \cinline{@*/}
(so that they are first and foremost, for almost all C tools, a regular C
comment). Importantly, for several reasons (NEEL\@: explain here? or later?),
these are checked on a \intro{per function} basis.
...
origins as an formal, executable specification for \kl{ISO} and \kl{de facto}
...
on the input. \cinline{MAXi32()} is an in-built function which represents the
maximum value a signed 32-bit integer can represent. By constraining the input
so that it is strictly less than the maxiumum value, the function is now
guaranteed to have no \kl{UB} for all its inputs, no matter what the context.
This is because whilst pre-conditions are \emph{assumed} inside the function,
they are \emph{required} when calling it.
...
100i32},\sidenote{Integer literals are currently written with a
type annotation, similar to Rust.} \kl{CN} deduces that \cinline{y <= MAXi32}
and permits the call to \cinline{increment}. Conversely, \cinline{INT_MAX} does
not meet that constraint, and so CN raises an error.
...
Kernel CN, heap factoring, and Ott engineering.
...
Tree Carver, buddy allocator update, working with Galois.
