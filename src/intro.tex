...
alias assumptions at play here. It guesses that \cinline{ADDRESS_PFI_1PG} is
the address of a variable local to function \cinline{f}, which is
...
has been \intro{exposed}: cast to an integer (but otherwise
...
later.\sidenote{This example does have \kl{UB} though because of (demonic)
allocation address non-determinism.}
...
\begin{figure*}[tp]
    ...
    Modifications include: writing \cninline{Owned<✓,τ>} and \cninline{Owned<x,τ>}
    instead of \cninline{RW<τ>} and \cninline{W<τ>} respectively; explicitly
    binding the return value in the \cninline{ensures} clause, with an
    identifier $y$, rather than assuming it is the keyword \cninline{return};
    allowing arbitrary nesting of specifications under \cninline{take}, instead
    of just predicates and iterated predicates; allowing \cninline{if}s at
    places other than the outermost level of a predicate
    definition.}\label{fig:cn-grammar}
\end{figure*}
...
definition consists of either a top-level \cninline{f} with a list of $\mathit{spec}$s
in each branch, or just the list of ${specs}$ at the top-level. ${specs}$ are
the ${conditions}$ followed by a return expression.\sidenote{\url{https://github.com/rems-project/cerberus/issues/266}}
