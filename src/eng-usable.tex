\chapter{\kl{CN} Comparison and Feedback}\label{chap:comparison-feedback}
...
Lastly, I will discuss some feedback given by some industry partners on
...
the aforementioned \kl{buddy allocator}. It is a simple allocator which just bumps a
...
The second function allocates a zeroed page. It accesses two global
...
Stepping back, \kl{VeriFast} uses an ad hoc close-to-syntax semantics of C the
developers believe to be sound. This has the advantage of allowing allow for
some interesting, albeit confusing, heuristics such as the array indexing
example. However, it has the downside of missing a long tail of features
required for verifying real-world code, such as the following.
\begin{itemize}
...
comparison? We see that separation logic, moded recursive predicates, lemmas,
pre- and postconditions, loop invariants, proof annotations, even the use of a
subset of C as a proof language, seem to come with the terrain of verifying C
code. \kl{CN}'s monadic syntax (\cref{sec:monadic-syntax}) sidesteps much of
the confusion related to modes; the limited expressiveness of only allowing
precise predicates (\cref{subsec:precise-assertion}) has
not affected substantial verifications, and in fact enabled runtime testing of
assertions with \kl{Fulminate}. \kl{CN} will need to improve its speed,
visibility into the proof state and interactive replay to make failed proofs
less painful to debug.

\kl{CN}'s decision to support \kl{iterated} separating conjunctions reduces the
need for lemmas and heuristics as \kl{VeriFast} often relies on, but does not
eliminate it, especially when inferring indices can slow down verification.
Automating the \emph{common cases} of indexing, predicate unfolding, and lemma
applications, whilst retaining reasonable performance, and a fallback to a fast
but verbose manual mode, would allow \kl{CN} to adopt some of the advantages of
\kl{VeriFast}'s concision, without risking confusion in (novice) users.
...
mentioned so far. This trust is undercut by its ad hoc semantics for C
...
In this thesis, I have framed \kl{CN} as a refinement typing system, but it is
...
focus only on tools which have some relation to \emph{verifying} C.

Almost all of the tools I discuss have some proof of soundness associated with
its approach, as well as some link to testing or inferring assertions. The
former is necessary for increasing expressiveness and reducing the TCB, and the
latter is necessary for improving usability and usefulness (by providing
incremental benefit for incomplete specifications).
...
calls to a certain depth) and compositional execution (with specifications of
pre- and postconditions on function). It supports reasoning in both
...
provides predicate transformers, existential quantification and separating
...
freeing resources partial maps and user-defined recursive predicates. For OX,
...
and the approach has been proven sound~\sidecite{loow2025compositional}.

% Caveat: folding (strictly exact) predicates in UX reasoning is unsound.

Note that, in \kl{CN}, the syntax guides the instantiation of existential
variables and output arguments, because variable scoping ensures
mode-correctness; the syntax also guides the order of consuming resources from
the context. Gillian instead opts for a more traditional separation-logic
syntax, and infers the order of the aforementioned items
automatically~\sidecite{loow2024matching}.
...
block trees}. Both the C model and the memory object model are ad hoc.
...
conceptually straightforward but the latter is quite unclear. Gillian-C's
whole-program symbolic testing and UX specification synthesis found bugs in the
Collections-C library, an open-source library of generic data-structure for C
...
execution, but uses a variant of separation logic, implicit dynamic frames
...
Memory arrays and pointers with offsets can be encoded, but pointer arithmetic
and byte-level reasoning is not directly supported. IDF separates out
\emph{accessibility} predicates \textemdash{} used to specify the shape of the
heap (permissions) \textemdash{} from \emph{heap-dependent functions}
\textemdash{} used to specify the contents of the heap (values). Like \kl{CN},
this separates input arguments as parameters and output arguments as the result
(of the functions). However, most of the time assertions must be self-framing
(include all permissions to evaluate the heap accesses in the assertion).
...
experimental proof-of-concept one for \kl{CN}. Soteria-\kl{CN} uses
...
size of the program, though iterated predicates, inductive datatype and pure
functions are unsupported, so bigger and more representative examples may not
be so impressive.

The C front-end uses Cerberus' Ail rather than Core, because Ail retains more
...
Cerberus will elaborate the \cinline{&&} operator (in Ail) to an if-expression
(in Core) to capture the fact that it is short-circuiting and may have
side-effects. To handle this possibility soundly, the system will fork the
symbolic execution path, even though this would be unnecessary for most code.
Another example is local variables: Cerberus models variables as heap-allocated
...
This is borne about by feedback from BAE Systems and Galois, industry partners
...
workflow~\sidecite{aamer2026code}.
