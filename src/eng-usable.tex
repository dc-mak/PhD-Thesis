...
block trees}. Both C and its memory object model are ad hoc.
...
conceptually straightforward but the latter is quite unclear. Gillian-C works
with real world C:\@ its whole-program symbolic testing and UX specification
synthesis found bugs in the Collections-C library, an open-source library of
generic data-structure for C programs; its OX specification checking was used
to verify encryption header parsing in the AWS Encryption SDK for C, and found
bugs along the way. However, its emphasis on automation entails a surprising
property: it is possible to slow down verification by adding more annotations.
