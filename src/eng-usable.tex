...
code. \kl{CN}'s monadic syntax sidesteps much of the confusion related to
modes; the limited expressiveness of only allowing precise predicates has not
affected substantial verifications, and in fact enabled runtime testing of
separation-logic assertions. \kl{CN} will need to improve its speed, visibility
into the proof state and interactive replay to make failed proofs less painful
to debug.

When compared to \kl{VeriFast} approach of working an ad-hoc, close-to-syntax,
semantics of C, \kl{CN}'s design of working above \kl{Cerberus}' \kl{Core} is a
mixed bag. The benefit is that many C features, including a long tail small but
required ones for real-world code, are supported well. Related, I conjecture
understanding and proving \kl{VeriFast} sound above its ad-hoc C semantics is
more difficult than for \kl{CN}. The cost is that the we impair our ability to
allow for some interesting, albeit confusing, heuristics such the array
indexing example.
