...
annotations on OpenCL/OpenMP/C code. \sidetextcite{blom2017vercors} says
...
size of the program, though \kl{iterated predicates}, inductive datatypes and \kl{pure
functions} are unsupported, so bigger and more representative examples may not
be so impressive.
...
In the last chapter, I compared \kl{CN} to some relevant tools in the space. I
also presented industry feedback on \kl{CN}. Based on my
experience of having worked on programming language tooling both in industry
and in academia, I will synthesise the two and lay out achievable standards of
usability for \kl{CN} \emph{going forward}, with an emphasis on how \emph{new
projects} can avoid several pitfalls. Only where I explicitly say so are
the suggestions intended to be retrospective.
...
my experience in implementing \kl{VIP}: writing examples and informally but
rigorously working through how they ought to be specified and helped enormously
in guiding my intuitions and highlighting problems early on, and I conjecture a
...
pointers} to the temporary objects to the callee. This %
leads to very verbose and awkward specifications, where functions had to (a)
claim ownership of their parameters and (b) promise they did not modify them so
that the objects at the call-site remained unaffected.
...
duplicated effort. Had we spent some time looking at small sample \k{Core}
...
\end{itemize}
If, like in \kl{CN} proof mode, the last (semantic analysis) stage is
...
to support multiple errors through multiple stages (and dependency analysis,
and feature gating), and there are concrete benefits to writing code to with
this in mind from the start.

Consider failed the bit-vector update for the \kl{buddy allocator}
(\cref{sec:buddy-failed-bv}). One of its major pain points was (tediously)
switching \emph{every integer in every datatype, function,
predicate, and pre/postcondition} to a bit-vector; without this, despite
being designed for \emph{per-function} checking, \kl{CN} would stop at the
first integer/bit-vector mismatch. On just this update, a few extra weeks of
implementation effort early on would have saved \emph{months} of scarce time:
\begin{itemize}
...
\kl{CN} has been developed for around 5 years at the time of writing. It has
...
the solver, which we believe to still be the case. However, we need better
...
are noisy and coarse \textemdash{} the script measures the total execution
...
impossible to update the \kl{pKVM} \kl{buddy allocator} to verify after \kl{CN}'s
...
unstructured debug output, whilst occasionally helpful, is rarely the first,
