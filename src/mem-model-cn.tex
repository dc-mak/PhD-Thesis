...
\begin{itemize}
    \item What will reading this chapter conclude in?
    \item Typing rules,
    \item Prove soundness of typing rules with respect to VIP
\end{itemize}
...
It is more fruitful to look at the tests individually, and see which ones show
a demonstrable slowdown when \kl{VIP} was enabled.
...
and the tail of a \cinline{snoc}'d list to which an is simply the head of the
...
reinterpret that as a ownership of a larger queue from \cinline{front} to
...
It does not however, supporting exporting and proving lemmas involving
...
\kl{CN-VIP} abstract states, upon which an Iris logic could be defined and
